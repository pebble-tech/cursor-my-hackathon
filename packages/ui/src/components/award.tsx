...
export function Awards({
      ...
      <div className={cn('relative mx-auto flex h-48 w-48 items-center justify-center', className)}>
        <svg className="absolute inset-0 h-full w-full" viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg">
          <defs>
            ...
            <path id="bottom-curve" d={createTextPath(60, 'bottom-curve')} fill="none" transform="rotate(180 96 96)" />
          </defs>
  ...
  if (variant === 'award') {
    ...
    return (
      <div
        className={cn('relative z-0', 'flex flex-col items-center justify-center p-6', 'overflow-hidden', className)}
      >
        <LaurelWreath />
        ...
        <div className={cn('rounded-md border-4 p-4', 'flex justify-start gap-3')}>
          ...
          <div className={cn('border-l px-3')}>
            ...
            <div className="mt-1 flex items-center gap-4 text-xs">
              {recipient && <p className={cn('italic')}>by {recipient}</p>}•<div className={cn('')}>{date}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={cn('bg-card flex items-center justify-center rounded-md border p-1 shadow-lg', className)}>
        <div className="relative h-100 w-72 overflow-hidden rounded-sm border-4">
          ...
          <div className="mt-24 px-6">
            ...
            <p className="text-muted-foreground text-md -mt-2 font-light tracking-wider">{subtitle}</p>
          </div>
